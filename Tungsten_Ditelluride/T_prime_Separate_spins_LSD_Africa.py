...
circle2 = pb.circle(radius=1.32* scale, center=[-1.7* scale, 0.66* scale]) # define a circle with radius 1nm
...
solver = pb.solver.arpack(model, k=320 , sigma=0.2) # solves for the k-number lowest energy eigen values around sigma 

eigenvalues = solver.eigenvalues
eigenvectors = solver.eigenvectors

# Define spin projectors
spin_up = []; spin_down = []

for i, val in enumerate(eigenvalues):

    vec = eigenvectors[:, i]          # eigenvector
    vec_spinor = vec.reshape(-1, 2)  # separate spin components
    weight_up = np.sum(np.abs(vec_spinor[:, 0])**2)
    weight_down = np.sum(np.abs(vec_spinor[:, 1])**2)

    if weight_up > weight_down:
        spin_up.append(val)
    else:
        spin_down.append(val)


spin_up = np.sort(np.real(spin_up))
spin_down = np.sort(np.real(spin_down))


spin_sets = [(spin_up, "Spin Up"), (spin_down, "Spin Down")]

fig, axs = plt.subplots(2, 2, figsize=(10, 6))

for row, (eigenvalues, title) in enumerate(spin_sets):
    N_E = np.arange(1, len(eigenvalues) + 1)  # Number of states below each energy level

    # Polynomial smoothing
    poly = Polynomial.fit(eigenvalues, N_E, deg=41)
    N_smooth = poly(eigenvalues)

    # Compute spacings between each level
    S_n = np.diff(N_smooth)
    S_n_norm = S_n / np.mean(S_n)

    Bin = round(np.sqrt(len(eigenvalues)))

    # --- Staircase function (left column) ---
    ax = axs[row, 0]
    ax.step(eigenvalues, N_E, where='post', label="Numerical $N(E)$")
    ax.plot(eigenvalues, N_smooth, linestyle='dashed', label="Smoothed $N(E)$")
    ax.set_xlabel("E"); ax.set_ylabel("N(E)")
    ax.set_title(f"Spectral Staircase Function ({title})")
    ax.legend(); ax.grid()

    # --- Level spacing distribution (right column) ---
    s = np.linspace(0, 6, 100000)
    ax = axs[row, 1]
    ax.hist(S_n_norm, bins=Bin, density=True, alpha=0.6, label="Numerical $P(S)$")
    ...
    ax.set_title(f"Level Spacing Distribution ({title})")
    ax.legend(); ax.grid()
