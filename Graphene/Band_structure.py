...
for n in range(int(k)):
    ...
    print(n,'out of', int(k)) #iterations counter to check progess in the loop


plt.figure()
plt.scatter(max_kx_values, max_ky_values)
plt.xlabel("k-space path"); plt.ylabel("Energy")

a = graphene.a # unit cell
Gamma = [0, 0]
K1 = [-4*pi / (3*a), 0]
K2 = [4*pi / (3*a),0]
points = np.array([Gamma, K1, K2])
labels = ['Γ', 'K1', 'K2']
